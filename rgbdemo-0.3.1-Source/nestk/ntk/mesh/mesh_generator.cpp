...
namespace ntk
{
  ...
  void MeshGenerator :: generate(const RGBDImage& image)
  {
    ...
    {
    case TriangleMesh:
      return generateTriangleMesh(image);
    case  PointCloudMesh:
      return generatePointCloudMesh(image);
    case SurfelsMesh:
      return generateSurfelsMesh(image);
    }
  }
  ...
  void MeshGenerator :: generateTriangleMesh(const RGBDImage& image)
  {
    ...
    for_all_rc(depth_im)
    {
      ...
      Point3f p3d = image.calibration()->depth_pose->unprojectFromImage(Point3f(c,r,depth));
      Point3f p2d_rgb;
      Point2f texcoords;
      if (m_use_color)
      {
        p2d_rgb = image.calibration()->rgb_pose->projectToImage(p3d);
