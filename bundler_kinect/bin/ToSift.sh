...
do 
    ...
    depth_file=$IMAGE_DIR/`echo $d | sed 's/color.jpg$/depth.raw/' | sed 's/jpg/raw/'`
    echo "mogrify -format pgm $IMAGE_DIR/$d; $SIFT < $pgm_file > $key_file; rm $pgm_file; $BIN_PATH/InjectDepth $key_file $depth_file; gzip -f $key_file"
