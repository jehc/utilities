...
KVOViewerWidget::initializeGL(QGLPainter * painter)
{
  ...
  std::cout << "Loaded kvo of size " << voxels.size (0) << " " << voxels.size (1) << " " << voxels.size (2) << std::endl;
...
KVOViewerWidget::initializeTexture(KVO & voxels)
{
  ...
  // copy buffer
  glTexImage3D (GL_TEXTURE_3D, 0, GL_ALPHA, voxels.size (0), voxels.size (2), voxels.size (1), 0, GL_ALPHA, GL_FLOAT, &buffer [0]);
...
KVOViewerWidget::paintGL (QGLPainter * painter)
{
  ...
  shaderProgram->setUniformValue (projectionMatrixLocation, camera()->projectionMatrix ((float)width()/height()));
  ...
  for (size_t i = 0; i < corners.size(); ++i)
  {
    if (corners[i][2] < min) min = corners[i][2];
  ...
  for (size_t i = 0; i < corners.size(); ++i)
  {
    if (voxelCorners[i][2] < voxelMin) voxelMin = voxelCorners[i][2];
    if (voxelCorners[i][2] > voxelMax) voxelMax = voxelCorners[i][2];
  }
...
#pragma omp parallel for
  for (size_t s = 0; s < slices.size(); ++s)
  {
    ...
    for (size_t i = 0; i < edges.size(); ++i)
    {
      ...
      Eigen::Vector3f b (corners[edge.first][0], corners[edge.first][1], corners[edge.first][2] - (min + s*sliceDelta));
      ...
      Eigen::Vector4f intersection = (corners[edge.first] + x[0]*(corners[edge.second] - corners[edge.first]));
  ...
  for (size_t i = 0; i < slices.size(); ++i)
  {
    ...
    for (size_t j = 0; j < intersections.size(); ++j)
    {
      intersections [j].first = atan2 (intersections [j].second[1] - center[1], intersections [j].second[0] - center[0]);
    }
