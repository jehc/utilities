...
void BPS::optimize_GRID_GENERAL(int nIterations)
{
	...
	for ( ; nIterations > 0; nIterations --)
	{
		...
		for (x=0; x<m_width; x++, D_ptr+=K, M_ptr+=2*K, V_ptr+=2*K*K, n++)
		{
			...
			if (x < m_width-1) 
			{
				UpdateMessageGENERAL(M_ptr, Di, K, 1, /* forward dir*/ 0, V_ptr, buf);
			}
			if (y < m_height-1) 
			{
				UpdateMessageGENERAL(M_ptr+K, Di, K, 1, /* forward dir*/ 0, V_ptr+K*K, buf);
			}
		}
		...
		for (x=m_width-1; x>=0; x--, D_ptr-=K, M_ptr-=2*K, V_ptr-=2*K*K, n--)
		{
			...
			if (x > 0) 
			{
				UpdateMessageGENERAL(M_ptr-2*K, Di, K, 1, /* backward dir */ 1, V_ptr-2*K*K, buf);
			}
			if (y > 0)
			{
				UpdateMessageGENERAL(M_ptr-(2*m_width-1)*K, Di, K, 1, /* backward dir */ 1, V_ptr-(2*m_width-1)*K*K, buf);
			}
		}
	}
