            ...
            std::vector<std::vector<int> > & normalizationMap)
{
  ...
  normalizationMap = std::vector<std::vector<int> > (normalBins->size(), std::vector<int> ((*normalBins)[0][0].size()));
  ...
  for (pcl::PointCloud<pcl::PointXYZRGBNormal>::const_iterator i = points.begin(); i != points.end(); ++i)
  {
    ...
    if (i->normal_x*i->normal_x + i->normal_y*i->normal_y + i->normal_z*i->normal_z < 1e-9)
    {
      std::cout << "bad normal at " << i->x << " " << i->y << " " << i->z << std::endl;
      continue;
    }
...
float
findFloor (const pcl::PointCloud<pcl::PointXYZRGBNormal> & points, float limit)
{
  float floor = 0;
  int n = 0;
  for (pcl::PointCloud<pcl::PointXYZRGBNormal>::const_iterator i = points.begin(); i != points.end(); ++i)
...
generateLayers (const std::vector<std::vector<std::vector<pcl::PointNormal> > > & normalBins, 
               std::vector<std::vector<cv::Mat> > * & positiveLayers,
               std::vector<std::vector<cv::Mat> > * & negativeLayers)
{
  positiveLayers = new std::vector<std::vector<cv::Mat> > (14, std::vector<cv::Mat> (normalBins[0].size()));
  negativeLayers = new std::vector<std::vector<cv::Mat> > (14, std::vector<cv::Mat> (normalBins[0].size()));
  #pragma omp parallel for
  for (int i = 0; i < positiveLayers->size(); ++i)
  {
    for (size_t j = 0; j < (*positiveLayers)[i].size(); ++j)
    {
      (*positiveLayers)[i][j] = 1e20*cv::Mat::ones (normalBins.size(), normalBins[0][0].size(), CV_32FC1);
      (*negativeLayers)[i][j] = 1e20*cv::Mat::ones (normalBins.size(), normalBins[0][0].size(), CV_32FC1);
    }
  }
  #pragma omp parallel for
  for (int i = 0; i < normalBins.size(); ++i)
...
//          (*positiveLayers)[regions[q]][j_prime].at<float>(i, k) = abs(j - j_prime) * ((*positiveLayers)[regions[q]][j_prime].at<float>(i, k) + 1);
          (*negativeLayers)[opposite(regions[q])][j].at<float>(i, k) = 0;//regions.size();
...
{
  outputLayers = new std::vector<std::vector<cv::Mat> > (inputLayers.size(), std::vector<cv::Mat> (inputLayers[0].size()));
  #pragma omp parallel for
  for (int i = 0; i < inputLayers.size(); ++i)
  {
    for (size_t j = 0; j < inputLayers[i].size(); ++j)
    {
      ...
      for (int y = 0; y < inputLayers[i][j].rows; ++y)
      {
        ...
        {
          (*outputLayers)[i][j].at<float>(y, x) = inputLayers[i][j].at<float>(y, x) < 1e20 ? 1 : 0;
        }
      }
    }
  }
                   ...
                   std::vector<std::vector<cv::Mat> > * & outputLayers)
{
  ...
  for (int i = 0; i < inputLayers.size(); ++i)
  {
    ...
    std::vector<float> f (std::max(std::max(width,height), (int)inputLayers[i].size()));
...
void
distanceTransform (const std::vector<std::vector<cv::Mat> > & inputPositiveLayers,
                   const std::vector<std::vector<cv::Mat> > & inputNegativeLayers,
                   std::vector<std::vector<cv::Mat> > * & outputLayers)
{
  ...
  outputLayers = new std::vector<std::vector<cv::Mat> > (posResult->size(), std::vector<cv::Mat> ((*posResult)[0].size()));
...
std::pair<int, int>
findResponse (const std::vector<std::vector<cv::Mat> > & targetLayers, int yOffset,
              const std::vector<std::vector<cv::Mat> > & sourceLayers,
              const std::vector<std::vector<int> > & targetNormalizationMap,
              const std::vector<std::vector<int> > & sourceNormalizationMap,
              float & max, cv::Mat & sum)
{
  assert (targetLayers.size() == sourceLayers.size());
  sum = cv::Mat::zeros(targetLayers[0][0].rows, targetLayers[0][0].cols, CV_32FC1);
  #pragma omp parallel for
  for (int i = 0; i < targetLayers.size(); ++i)
  {
    for (size_t j = 0; j < targetLayers[i].size() - yOffset && j < sourceLayers[i].size(); ++j)
    {
      cv::Mat convolved (targetLayers[i][j].rows, targetLayers[i][j].cols, CV_32FC1);
      cv::Mat filter (sourceLayers[i][j].rows, sourceLayers[i][j].cols, CV_32FC1);
      cv::flip (sourceLayers[i][j], filter, -1);
      cv::filter2D (targetLayers[i][j + yOffset], convolved, -1, filter, cv::Point(0, 0), 0, cv::BORDER_CONSTANT);
      #pragma omp critical
      {
        sum += convolved;
      }
...
void saveLayers_char (const std::string & filename, const std::vector<std::vector<cv::Mat> > & layers)
{
  pcl::PointCloud<pcl::PointXYZRGBNormal> cloud;
  #pragma omp parallel for
  for (int i = 0; i < layers.size(); ++i)
  {
    cv::Mat sum = cv::Mat::zeros (layers[i][0].rows, layers[i][0].cols, CV_32FC1);
      ...
      for (int k = 0; k < layers[i][j].rows; ++k)
      {
        for (int l = 0; l < layers[i][j].cols; ++l)
        {
          ...
          if (layers[i][j].at<char>(k, l))
          {
            ...
            point.normal_x = normal.normal_x*layers[i][j].at<char>(k, l)/127;
            point.normal_y = normal.normal_y*layers[i][j].at<char>(k, l)/127;
            point.normal_z = normal.normal_z*layers[i][j].at<char>(k, l)/127;
...
void saveLayers_float (const std::string & filename, const std::vector<std::vector<cv::Mat> > & layers)
{
  pcl::PointCloud<pcl::PointXYZRGBNormal> cloud;
  #pragma omp parallel for
  for (int i = 0; i < layers.size(); ++i)
  {
    ...
    std::stringstream ss;
    ss << "/home/kmatzen/NOBACKUP/" << filename << ".layer_dist" << i << ".txt";
    saveResponseMap (ss.str(), sum);
  }
  savePlyFile (filename, cloud);
}
...
void
saveNormalizationMap (const std::string & filename, const std::vector<std::vector<int> > & normalMap)
{
  std::ofstream output (filename.c_str());
  if (!output)
  {
    std::cout << "failed to open file " << filename << " for write" << std::endl;
    exit (1);
  }
...
main (int argc, char ** argv)
{
  ...
  if (argc < 5)
  {
    std::cout << "Usage: " << argv[0] << " [target].pcd [source].pcd [voxel_size] [output]" << std::endl;
    return -1;
  }
  pcl::PointCloud<pcl::PointXYZRGBNormal> source;
  pcl::PointCloud<pcl::PointXYZRGBNormal> * target = new pcl::PointCloud<pcl::PointXYZRGBNormal>();
...
*/
  ...
  std::vector<std::vector<std::vector<pcl::PointNormal> > > * targetNormalBins = 0; 
  ...
  gettimeofday (&tic, NULL);
  int yOffset = binNormals (*target, lengthOfEdge, targetMin, targetMax, targetNormalBins, targetNormalizationMap);
  delete target;
  target = 0;
  gettimeofday (&toc, NULL);
  std::cout << " complete in " << toc.tv_sec - tic.tv_sec << " seconds" << std::endl;
  ...
  saveNormalizationMap ("/home/kmatzen/NOBACKUP/normalization.txt", targetNormalizationMap);
#endif
  ...
  gettimeofday (&tic, NULL);
  generateLayers (*targetNormalBins, targetLayersPositiveBinary, targetLayersNegativeBinary);
  ...
  gettimeofday (&tic, NULL);
  distanceTransform (*targetLayersPositiveBinary, *targetLayersNegativeBinary, targetLayersFilter);
  ...
  for (int m = 0; m < 8; ++m)
  {
    ...
    pcl::PointCloud<pcl::PointXYZRGBNormal> * sourceTrans = new pcl::PointCloud<pcl::PointXYZRGBNormal>();

    for (pcl::PointCloud<pcl::PointXYZRGBNormal>::const_iterator j = source.begin(); j != source.end(); ++j)
    {
      pcl::PointXYZRGBNormal newPoint = *j;
      float x = trans.at<float> (0, 0)*newPoint.x + trans.at<float> (0, 1)*newPoint.y + trans.at<float> (0, 2)*newPoint.z;
      float y = trans.at<float> (1, 0)*newPoint.x + trans.at<float> (1, 1)*newPoint.y + trans.at<float> (1, 2)*newPoint.z;
      float z = trans.at<float> (2, 0)*newPoint.x + trans.at<float> (2, 1)*newPoint.y + trans.at<float> (2, 2)*newPoint.z;
      float nx = trans.at<float> (0, 0)*newPoint.normal_x + trans.at<float> (0, 1)*newPoint.normal_y + trans.at<float> (0, 2)*newPoint.normal_z;
      float ny = trans.at<float> (1, 0)*newPoint.normal_x + trans.at<float> (1, 1)*newPoint.normal_y + trans.at<float> (1, 2)*newPoint.normal_z;
      float nz = trans.at<float> (2, 0)*newPoint.normal_x + trans.at<float> (2, 1)*newPoint.normal_y + trans.at<float> (2, 2)*newPoint.normal_z;
      newPoint.x = x;
      newPoint.y = y;
      newPoint.z = z;
  ...
  std::vector<std::vector<std::vector<pcl::PointNormal> > > * sourceNormalBins = 0;
  ...
  gettimeofday (&tic, NULL);
  binNormals (*sourceTrans, lengthOfEdge, sourceMin, sourceMax, sourceNormalBins, sourceNormalizationMap);
  delete sourceTrans;
  sourceTrans = 0;
  gettimeofday (&toc, NULL);
  std::cout << " complete in " << toc.tv_sec - tic.tv_sec << " seconds" << std::endl;
  ...
  gettimeofday (&tic, NULL);
  generateLayers (*sourceNormalBins, sourceLayersPositiveBinary, sourceLayersNegativeBinary);
...
//  distanceTransform (sourceLayersPositiveBinary, sourceLayersNegativeBinary, sourceLayersFilter);
  ...
  float max_temp = 1e20;
  cv::Mat sum;
  std::pair<int, int> response = findResponse (*targetLayersFilter, yOffset, *sourceLayersFilter, targetNormalizationMap, sourceNormalizationMap, max_temp, sum);
  ...
  #pragma omp critical
  {
    ...
    std::cout << response.first << " " << response.second << " " << max_temp << std::endl;
  }
  gettimeofday (&toc, NULL);
  std::cout << " complete in " << toc.tv_sec - tic.tv_sec << " seconds" << std::endl;
  ...
  for (pcl::PointCloud<pcl::PointXYZRGBNormal>::const_iterator i = source.begin(); i != source.end(); ++i)
  {
    ...
    float x = max_trans.at<float> (0, 0)*newPoint.x + max_trans.at<float> (0, 1)*newPoint.y + max_trans.at<float> (0, 2)*newPoint.z;
    float y = max_trans.at<float> (1, 0)*newPoint.x + max_trans.at<float> (1, 1)*newPoint.y + max_trans.at<float> (1, 2)*newPoint.z;
    float z = max_trans.at<float> (2, 0)*newPoint.x + max_trans.at<float> (2, 1)*newPoint.y + max_trans.at<float> (2, 2)*newPoint.z;
    float nx = max_trans.at<float> (0, 0)*newPoint.normal_x + max_trans.at<float> (0, 1)*newPoint.normal_y + max_trans.at<float> (0, 2)*newPoint.normal_z;
    float ny = max_trans.at<float> (1, 0)*newPoint.normal_x + max_trans.at<float> (1, 1)*newPoint.normal_y + max_trans.at<float> (1, 2)*newPoint.normal_z;
    float nz = max_trans.at<float> (2, 0)*newPoint.normal_x + max_trans.at<float> (2, 1)*newPoint.normal_y + max_trans.at<float> (2, 2)*newPoint.normal_z;
    newPoint.x = x;
    newPoint.y = y;
    newPoint.z = z;
    newPoint.normal_x = nx;
    newPoint.normal_y = ny;
    newPoint.normal_z = nz;
    newPoint.x += lengthOfEdge*max_coord.first - sourceMin_max [0] + targetMin [0];
    newPoint.y += lengthOfEdge*yOffset -sourceMin_max [1] + targetMin [1];
    newPoint.z += lengthOfEdge*max_coord.second - sourceMin_max [2] + targetMin [2];
